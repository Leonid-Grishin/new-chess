...
@section('breadcrumbs')
    <ol class="breadcrumb bg-transparent mb-0">
        ...
        <li class="breadcrumb-item active" aria-current="page">Добавление Отзыва</li>
    </ol>
@endsection
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="mb-2">
                <h1 class="h1 text-center">Добавление отзыва</h1>
            </div>
        </div><!-- /.container-fluid -->

        <form enctype="multipart/form-data" method="post" action="{{ route('admin.reviews.store') }}">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <video class="mt-2" src="#" id="uploadedVideo" controls width="340"></video>
                </div>
                <div class="form-group">
                    <label for="inputName">Имя</label>
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="inputName" name="name" placeholder="Добавьте имя" value="{{ old('name') }}">
                    @error('name')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="inputDescription" name="description" placeholder="Добавьте краткую суть отзыва" rows="3">{{ old('description') }}</textarea>
                    @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
